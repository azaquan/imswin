...
If SSoleEccn.IsAddRow Then
   ...
   If colnum = 1 Then
    ...
    If colnum = 1 Then
      ...
      If Len(SSoleEccn.Columns(colnum).Text) > 10 Then
      
         RecSaved = False
         msg1 = translator.Trans("M00702")
         MsgBox IIf(msg1 = "", "Source Field can not be greater than 10.", msg1)
         SSoleEccn.SetFocus
         SSoleEccn.Col = colnum
         validate_fields = False
         Exit Function
    ...
    End If
      ...
      If colnum = 1 Then
        ...
        x = DoesSourceExist(SSoleEccn.Columns(1).Text)
...
Do While Not Rseccn.EOF
    ...
    SSoleEccn.AddItem Rseccn("sourceid") & vbTab & Rseccn("source") & vbTab & Rseccn("source_desc") & vbTab & Rseccn("source_active")
    Rseccn.MoveNext
...
Dim ret As Integer
          ...
          If SSoleEccn.IsAddRow And ColIndex = 1 Then
             ...
             If NotValidLen(SSoleEccn.Columns(ColIndex).Text) Then
              ...
              ElseIf DoesSourceExist(SSoleEccn.Columns(ColIndex).Text) Then
                
                msg1 = translator.Trans("M00703")
                MsgBox IIf(msg1 = "", "Source already exists. Please create a unique one.", msg1)
                Cancel = 1
                SSoleEccn.SetFocus
                SSoleEccn.Columns(ColIndex).Text = oldVALUE
        ...
        ElseIf ColIndex = 2 Then
              ...
              If NotValidLen(SSoleEccn.Columns(ColIndex).Text) Then
                ...
                msg1 = translator.Trans("M00702")
                MsgBox IIf(msg1 = "", "Please enter a description for the Source.", msg1)
                Cancel = 1
                SSoleEccn.SetFocus
                RecSaved = False
                SSoleEccn.Col = 1
...
Private Function DoesSourceExist(Source As String) As Boolean
...
On Error GoTo ErrHand
    ...
    With cmd
        ...
        .CommandText = "DoesSourceExist"
        .CommandType = adCmdStoredProc
        .ActiveConnection = deIms.cnIms
        .parameters.Append .CreateParameter("@source", adVarChar, adParamInput, 25, Trim(Source))
        .parameters.Append .CreateParameter("@npecode", adVarChar, adParamInput, 5, deIms.NameSpace)
        Set rst = .Execute
    
        DoesSourceExist = rst!countit
...
Public Function SaveRecord(Delete As Boolean) As Boolean
        ...
        With cmd
            ...
            .ActiveConnection = deIms.cnIms
            .CommandText = "SOURCEUpdate"
            .CommandType = adCmdStoredProc
            .parameters.Append .CreateParameter("@sourceid", adBigInt, adParamInputOutput, , IIf(FormMode = mdCreation, 0, SSoleEccn.Columns(0).value))
            .parameters.Append .CreateParameter("@npecode", adVarChar, adParamInput, 5, deIms.NameSpace)
            .parameters.Append .CreateParameter("@source", adVarChar, adParamInput, 25, SSoleEccn.Columns(1).value)
            .parameters.Append .CreateParameter("@source_desc", adVarChar, adParamInput, 500, SSoleEccn.Columns(2).value)
            .parameters.Append .CreateParameter("@actiondelete", adBoolean, adParamInput, , Delete)
            .parameters.Append .CreateParameter("@source_active", adBoolean, adParamInput, , SSoleEccn.Columns(3).value)
        ...
        If FormMode = mdCreation Then SSoleEccn.Columns(0).value = cmd.parameters("@sourceid").value
        If Delete = True Then candelete = cmd.parameters("@CanDelete").value
        
        If candelete = 1 Then
                MsgBox "This source of information has records associated with Stockmaster\ Purchase Order and can not be deleted. De-activating it will stop users from including it in any new records.", vbInformation
                Exit Function
        End If
